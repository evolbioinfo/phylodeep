...
TRAINED_ON_LARGE_NN = 'predict huge trees, trained on large trees'
...
if __name__ == "__main__":
    ...
    types = sorted(df['type'].unique(),
                   key=lambda _: (0 if 'predict large' in _ else 1, 0 if 'subtree' in _ else 1, 0 if 'FFNN' in _ else 1))
    ...
    for type in types:
        for par in pars:
            if TRAINED_ON_LARGE_NN not in type:
    ...
    large_NN_type = [_ for _ in types if TRAINED_ON_LARGE_NN in _]
    for _ in large_NN_type:
        df = df.loc[df['type'] != _, :]
    types = [_ for _ in types if TRAINED_ON_LARGE_NN not in _]
